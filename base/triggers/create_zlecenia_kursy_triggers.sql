...
begin
    ...
    if (select count(*) from zlecenia_kursy zk where zk.id_zlecenia = new.id_zlecenia) = 0 then 
        if (select count (*) from kursy_paczkomaty kp join paczkomaty p on kp.id_paczkomatu = p.id_paczkomatu join zlecenia z on p.id_paczkomatu = z.id where z.id_zlecenia = new.id_zlecenia and kp.id_kursu = new.id_kursu and kp.data_odjazdu is null) > 0 then return new; end if;
